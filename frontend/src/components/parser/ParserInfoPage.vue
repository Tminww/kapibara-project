  ...
  <v-container fluid>
    ...
    <v-dialog v-model="stopDialog" max-width="500">
      <v-card class="px-2 py-2">
        ...
        <v-card-text>
          Вы уверены, что хотите <strong>остановить</strong> процесс сбора данных? Это действие
          необратимо.
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" variant="tonal" @click="stopDialog = false"> Отменить </v-btn>
          <v-btn color="error" variant="tonal" @click="confirmStopParser"> Остановить </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-container>
