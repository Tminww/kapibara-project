...
def get_document_api(code):
    ...
    if query.get_total_documents(code=code) == total_documents_data.get(
        "itemsTotalCount"
    ):
    ...
    for npa in type_data:
        ...
        while True:
            ...
            if req.status_code != 200:
                logging.error(
                    f"Ошибка при запросе документов: статус {req.status_code}"
                )
                break
            ...
            # Проверка, нужно ли обновлять данные
            if query.get_total_documents_type(
                code=code, npa_id=npa["id"]
            ) == documents_data.get("itemsTotalCount"):
                break
